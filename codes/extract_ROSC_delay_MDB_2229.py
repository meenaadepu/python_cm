...
with open(r'D:\Meena\Ltv\22LTV\config\2229\2229_ROSC_Extract_config.json') as f:
    config = json.load(f)
...
def get_die_filter_data(df,config, process):
    prc_config = [elem for elem in config['wafer_multi_uhvtidsat'] if elem['process'] == process]
    ...
    for data in prc_config:
        ...
        if not data['die_values']:  # If die_values are empty
            ...
            df_filter = df[(df['Wafer'] == data['alias']) &
            ...
            for dx, dy in data['die_values']:
                ...
                df_filter = df[(df['Wafer'] == data['alias']) & (df['HardBin'] == 1) & (df['SoftBin'] == 1) &
...
for sub_config in config['rosc_delay_config']:
  ...
  #  df1.to_csv(r"D:\Ltv\22LTV\2226\outputs\2226_5per_diefilter.csv")
    ...
    rosc_test = df.filter(regex=('ROSC_DELAY.*'), axis=1)
    print(rosc_test.shape)
    test_set = set([x.split(':')[0] for x in list(rosc_test)])
    print(test_set)
    f_name = ((sub_config['process'] + '_VDD' + str(sub_config['vdd']) + 'V_VNW' + str(sub_config['vnw']) + 'V_VPW' +
               str(sub_config['vpw']) + 'V_' + str(sub_config['temp'])).replace('.', 'p').replace('-', 'N')) + "C.xlsx"
    ...
    for test_name in test_set:
        ...
        test_volt = set([z.split('__r')[0] for z in test_grp])  # unique voltage
       #print(test_volt)
